<template>
  <n-tooltip trigger="hover" >
    <template #trigger>
      <n-button strong  round :="getBtnStyle()" @click="handleVimStatusBarClick()">
        <template #icon>
          <svg :fill="currentMode <= CODE_VIEW? configManager.getGlobalColor():'white'"
	...
	C478.5,179.8,484,147.3,476.9,114z"></path></svg>
        ...
        {{selectIndex}}{{subItemSelectIndex > -1? `#${subItemSelectIndex}`:''}}
      </n-button>
    </template>
    {{(currentMode <= CODE_VIEW)? 'Utool输入框失去焦点，才能成功启用Vim模式':'Vim模式不适用当前场景'}}
  </n-tooltip>
</template>
...
const getBtnStyle = ()=>{
  if(currentMode.value <= CODE_VIEW){
    return {
      color: utools.isDarkColors()? '#444444':'#e2e2e2',
