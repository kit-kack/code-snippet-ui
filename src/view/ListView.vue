<template>
  <template v-if="list.length > 0">
    <n-scrollbar style="max-height: 99vh" ref="scrollBar">
      <div ref="scroll_top">
        ...
        <div id="info" v-if="fullScreenShow">
          <p>~共有{{list.length}}条数据~</p>
        </div>
      </div>
    </n-scrollbar>
  </template>
  <template v-else-if="fullScreenShow">
  ...
  <!--  更新表单 页面-->
  <template v-if="currentMode === LIST_VIEW && fullScreenShow">
    <div id="extra">
      <n-space vertical>
        <n-tooltip trigger="hover" placement="left">
          <template #trigger>
            <n-button strong secondary circle type="primary" :color="configManager.getGlobalColor()" @click="currentMode = CREATE_VIEW" >
              <template #icon>
        ...
        <n-tooltip trigger="hover" placement="left" >
          <template #trigger>
            <n-button strong secondary circle type="primary" :color="configManager.getGlobalColor()" @click="emit('sideShow')">
              <template #icon>
        ...
        <n-tooltip trigger="hover" placement="left" >
          <template #trigger>
            <n-button strong secondary circle type="primary" :color="configManager.getGlobalColor()" @click="emit('refresh')">
              <template #icon>
  ...
  scrollListInvoker.value = (direction)=>{
    switch (direction){
      ...
      case "right":
        ...
        break;
      default:
        scrollBar.value.scrollTo?.({top:direction,left:0,behavior:'smooth'})
        break;
    }
  }
})
onUpdated(()=>{
  console.log(fullScreenShow.value)
...
const handleScroll = ()=>{
  let value = -scroll_top.value?.getBoundingClientRect?.().top;
  if(value == null || value < 50){
    value = 0;
  }
  scrollbarMovedDistance.distance = value;
}
