<template>
  <div v-if="$reactive.view.refresh" :style="{cursor: ($reactive.view.cursorShow)? '': 'none'}"
    ...
    <template v-else-if="$reactive.view.fullScreenShow">
    ...
    <div id="extra" v-if="$reactive.view.fullScreenShow" @mouseenter="expanded = true" @mouseleave="expanded = false">
      <template v-if="expanded || $reactive.view.buttonFixed">
        <n-space vertical style="padding-bottom: 6px">
          <n-tooltip trigger="hover" placement="left">
            <template #trigger>
              <n-button strong secondary circle type="primary" :color="configManager.getGlobalColor()" @click="goToCreateView()">
                <template #icon>
          ...
          <n-tooltip trigger="hover" placement="left">
            <template #trigger>
              <n-button strong circle secondary type="primary"  :color="configManager.getGlobalColor()"  @contextmenu="$reactive.view.buttonFixed = !$reactive.view.buttonFixed" @click="refreshListView()">
                <template #icon>
